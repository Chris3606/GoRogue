...
All value types in GoRogue 3 are immutable, and immutable value types such as `Point` and `Rectangle` do implement `Equals`, `GetHashCode`, and `IEquatable<T>` as they previously did.  However, they also implement `IMatchable` in a way equivalent to their `Equals` function, for consistency.
