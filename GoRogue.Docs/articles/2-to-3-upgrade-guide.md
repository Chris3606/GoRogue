...
One of the biggest takeaways from this is that it removes support for .NET Framework entirely, as .NET Framework does not support .NET Standard 2.1 and there are no future plans for it to do so.  Microsoft has [officially pivoted away](https://devblogs.microsoft.com/dotnet/announcing-net-5-0/) from adding new features to .NET Framework, and has committed to providing equivalent platform and feature support to their new .NET unified platform (previously .NET Core), and GoRogue 3 does support .NET 5+ and .NET Core 3.0+.
...
One additional benefit of this change is taht GoRogue 3 has been annotated to support [nullable reference types](https://docs.microsoft.com/en-us/dotnet/csharp/nullable-references) which were introduced in C# 8.  GoRogue's entire API now has annotations indicating proper nullability.
...
# Some Core Features Moved to TheSadRogue.Primitives
...
`TheSadRogue.Primitives` is open source, and maintained by myself and [Thraka](https://github.com/Thraka), and is compatible with all platforms that GoRogue is.  Additionally, GoRogue depends on this library, so it will be automatically installed via NuGet when GoRogue is.  Many changes will simply involve changing namespaces referred to from the GoRogue namespaces items were in, to `SadRogue.Primitives`.

The primitives library also has some basic classes that weren't in GoRogue 2, such as `Color`, `Gradient`, `Pallette`, and `PolarCoordinate`.  These may be useful to replace equivalent types.

The primitives library also has extension packages that allow its primitive types to easily convert to the equivalent primitive types in MonoGame, SFML, and some others; none of these extensions are required to use GoRogue but may be useful if you are using those libraries with GoRogue.
...
In GoRogue 2, types such as `Coord` and `Rectangle` had implicit conversions defined to their equivalent types in other libraries such as MonoGame, so that if you were using those libraries it was easy to convert between the two.  These conversions were included in such a way as to not create a hard dependency; unfortunately, some compilers were unable to parse the tags properly and tried to use these libraries as hard dependencies anyway.  Thus, "MinimalDependency" versions were created and released for users that ran into these scenarios, which made getting started more complex.

In GoRogue 3, the implicit conversions have been removed.  The types that had such conversions were moved to TheSadRogue.Primitives, and the conversions themselves have been replaced by the extension packages for `TheSadRogue.Primitives` for various libraries that define similar conversion functions.  See that library's documentation for details.
...
In GoRogue 2, a number of mutable reference types implemented `Equals`, `operator==`, and `GetHashCode`, such as `MapArea`.  However, according to [Microsoft guidance](https://docs.microsoft.com/en-us/dotnet/api/system.object.equals?view=net-5.0), types that are mutable should not implement `Equals`, as common uses of that hash (like using instances as keys in a `Dictionary`) require that the hash not change while it's being used.  Since `Equals`, `operator==` and `GetHashCode` are supposed to be implemented in tandem for things to work correctly, such classes should ideally not implement any of those functions.
...
## Renamed to GridViews and Moved to TheSadRogue.Primitives
...
These classes may optionally be inherited from as an alternative to `IGridView` and `ISettableGridView`, respectively.  For cases where there is nothing preventing your custom implementations from inheriting from a base class, this is much more convenient, as you need only implement the properties, and a single indexer; the other indexers are implemented based on that one.
...
The names of functions on the component collection class have also been modified to bring them more in-line with traditional C# collection names.  A mapping of equivalent functions is as follows:
- `AddComponent` -> `Add`
...
Note that `GetFirstOrDefault` most closely replicates the behavior of GoRogue 2's `GetComponent`; however, there is an additional function called `GetFirst` that is now provided.  This function throws an exception if a component of the specified type has not been found, and may be useful to replace cases where you're checking if a component is null and erroring immediately after retrieving it.
...
The factory system in GoRogue has been refactored to address some naming/usability concerns.  First, note that the `GoRogue.Factory` namespace is now `GoRogue.Factories`, which more accurately matches naming conventions elsewhere in the library.

The remaining factory changes fall in line with splitting out the class structures for the two main use cases of factories; to create items when additional configuration information per instance is needed, and when no such additional information is needed.  In GoRogue 2, both use cases were supported but the class structure for supporting this was somewhat unintuitive; it utilized an arbitrary class `BlueprintConfig` for a base configuration object, that served no purpose other than to provide a default value.  In GoRogue 3, the factory classes have been split out as follows:
- `Factory<TProduced>`
...
The map generation system has been completely rewritten from the ground up in GoRogue 3.  It functions in a notably different manner, so it is recommended that you review [this article](~/articles/map-generation.md) which explains the new system concepts and components.
...
### Quick Generators
The "full-map generation" algorithms that were present in GoRogue `GoRogue.MapGeneration.QuickGenerators` are represented in GoRogue 3 as functions in `GoRogue.MapGeneration.DefaultAlgorithms`, which produce a set of generation steps that, when executed in a `Generator`, follow a roughly equivalent process:
...
### Other Room/World Generation Algorithms
Most other GoRogue 2 map generation algorithms exist in some form as generation steps in the `MapGeneration.Steps` namespace.  The following is a rough mapping:
- `MapGeneration.MapAreaFinder` -> Unchanged
    - An actual map generation step that wraps this functionality is available in `MapGeneration.Step.AreaFinder`
- `MapGeneration.MapArea` -> `SadRogue.Primitives.Area`
- `MapGeneration.BasicRoomsGenerator`
    - Replaced by the new implementation of `MapGeneration.Steps.RoomsGeneration`
    - Very similar to GoRogue 2's `MapGeneration.RoomsGenerator` (see below)
    - The algorithms are similar enough that they could be reasonably substituted for each other
- `MapGeneration.CellularAutomataAreaGenerator`
    - Replaced by a combination of the `MapGeneration.Steps.RandomViewFill` (to randomly fill walls/floors) and `MapGeneration.Steps.CellularAutomataAreaGeneration` to do the smoothing
...
### Translation Steps
The article on map generation refers to "translation steps" as steps that purely transform data in the map generation context, as opposed to generating new data.  It may definately be helpful to review these steps, located in the `MapGeneration.Steps.Translation` namespace, as they may be useful in general cases.
...
Another change to spatial maps is the separation of moving _all_ items at a location vs. moving only the ones that can move.  In addition to `Move(T item)`, `ISpatialMap` now also contains `MoveAll` and `MoveValid` functions.  `MoveAll` attempts to move all functions at a given location to the target location, throwing an `InvalidOperationException` if this fails.  `MoveValid` only moves the items that can move, returning precisely which items were moved.
...
# GameFramework Namespace Classes Refactored
...
The class structure for `GameFramework.GameObject` has been simplified in GoRogue 3.  In GoRogue 2, support for use cases where you were unable to inherit from `GameObject` was somewhat complex.  You had to implement `IGameObject` in these cases; but, since implementation of the functionality of `IGameObject` was closely coupled with the code for `GameFramework.Map`, this was non-trivial.  The approach recommended for GoRogue 2 was to use a `GameObject` instance as a private backing field for your `IGameObject` implementation; and in fact GoRogue 2 had special support for this built into `GameObject` in the form of the `parent` parameter to the constructor.
...
## Functionality for Adding/Removing/Moving Entities Throws Exceptions
In GoRogue 3, spatial maps have been changed to throw exceptions in cases where invalid operations are performed (see the [corresponding section](#spatial-map-interface-refactored)).  Because `GameFramework.Map` is partially based upon spatial maps, and it exposes a number of similar operations via its API, similar changes have been made to `Map` and `IGameObject`.
