...
{
    /// <summary>
    /// Interface for pseudo-random number generators to implement.  TODO:, need to include ways for doubles, etc.
    /// Having this as an interface can be useful, as an RNG can be easily swapped out for testing.  There are classes included
    /// that implement this interface that are based on certain forms of pseudo-random number generation, as well as classes
    /// that implement it to do something that is not actually random (KnownSeriesRandom), etc, which can be useful for testing.
    /// </summary>
