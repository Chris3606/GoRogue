...
namespace GoRogue
{
    ...
    ///
    /// More detail: typical roguelikes will use a 2D array (or 1D array accessed as a 2D array), for terrain, and lists of objects for
    /// things like entities, items, etc.  This is simple but ultimately not efficient; for example, in that implementation, determining if there is an object at a location takes an amount of time
    /// proportional to the number of objects in this list.  However, the other simple option is to use an array with size equal to the size of the map (as many do for terrain) for all object lists.  This
    /// is even less ideal, as in that case, the time to iterate over all objects becomes proportional to the size of the map (since one has to do that for rendering, ouch!),
    /// which is typically much larger than the number of objects in a list.  This is the problem SpatialMap is designed to solve.  It provides fast, near-constant-time operations for getting the object
    /// at a location, adding entities, removing entities, and will allow you to iterate through all objects in the SpatialMap in time proportional to the number of objects in it (the best possible).
    ///
    /// Effectively, it is a more efficient list for objects that have a position associated with them.  This implementation
    /// can only allow one item at a given location at a time -- for an implementation that allows multiple items, see
    /// MultiSpatialMap.
    ///
    /// The objects stored in a SpatialMap must implement the IHasID (see that interface's documentation for an easy implementation example).  This is used internally to keep track of the objects, since
    /// uints are easily hashable.
    /// </summary>
    ...
    public class SpatialMap<T> : ISpatialMap<T> where T : IHasID
    {
        ...
        /// </summary>
        /// <param name="initialCapacity">The initial maximum number of elements the SpatialMap can hold before it has to internally resize data structures.  Defaults to 32.</param>
        ...
        /// <summary>
        /// Adds the given item at the given position, provided the item is not already in the SpatialMap and the position is not already filled.  If either of those are the case, returns
        /// false.  Otherwise (if item was successfully added), returns true.
        /// </summary>
        ...
        /// <summary>
        /// Move the item specified to the position specified.  Returns true if successful.  If the item does not exist in the SpatialMap, or the position is already filled by something, does nothing
        ...
        /// <summary>
        /// Moves whatever is at position current, if anything, to postion target.  If something was moved, returns what was moved.  If nothing was moved, eg. either there was nothing at position
        /// current or already something at position target, returns nothing.
        /// </summary>
        /// <remarks>
        /// Since this implementation of ISpatialMap guarantees that only one item may be at any given location at a time,
        /// the returned values will either be none, or a single value.
        /// </remarks>
        /// <param name="current">The position of the item to move.</param>
        /// <param name="target">The position to move the item to.</param>
        /// <returns>The item moved as a 1-element IEnumerable if something was moved, or nothing if no item was moved.</returns>
        ...
        /// </summary>
        /// <remarks>
        /// Again, since this implementation guarantees that only one item can be at any given location at a time, the
        /// returned value is guaranteed to be either nothing or a single element.
        /// </remarks>
        /// <param name="position">The position of the item to remove.</param>
        /// <returns>The item removed as a 1-element IEnumerable, if something was removed; nothing if no item was found at that position.</returns>
        ...
        /// </summary>
        /// <remarks>
        /// Again, since this implementation guarantees that only one item can be at any given location at once, the return
        /// value is guaranteed to be at most one element.  For times when you are know you are dealing exclusively with
        /// SpatialMap instances, this specific class also provides a GetItem function that returns a more intuitive null if
        /// no item was found, or the item at the location as applicable.
        /// </remarks>
        /// <param name="position">The position to return the item for.</param>
        /// <returns>The item at the given position as a 1-element enumerable, if there is an item there, or nothing if there is no item there.</returns>
        ...
        /// </summary>
        /// <remarks>
        /// Intended to be a more convenient function as compared to GetItems, for times when you are dealing exclusively
        /// with SpatialMap instances.
        /// </remarks>
        /// <param name="position">The postiion to return the item for.</param>
        /// <returns>The item at the given position, or null/equivalent if no item exists at that location.</returns>
