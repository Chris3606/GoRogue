...
namespace GoRogue.Messaging
{
	...
	public class MessageBus
	{
		...
		/// <see cref="UnregisterSubscriber{TMessage}(ISubscriber{TMessage})"/> when they are disposed of, to avoid the bus preventing
		...
		/// barring the case detailed in the <see cref="ISubscriber{TMessage}"/> remarks where one class subscribes to multiple message types.</typeparam>
		/// <param name="subscriber">Subscriber to add.</param>
		/// <returns>The subscriber that was added, in case a reference is needed to later call <see cref="UnregisterSubscriber{TMessage}(ISubscriber{TMessage})"/>.</returns>
