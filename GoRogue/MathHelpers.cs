...
namespace GoRogue
{
    ...
    public static class MathHelpers
    {
        ...
        /// <summary>
        /// A modified modulo operator.  Computes similar to num % wrapTo, following the formula (num % wrapTo + wrapTo) % wrapTo.
        /// Practically it differs from regular modulo in that the values it returns when negative values for num are given wrap around like
        /// one would want an array index to (if wrapTo is list.length, -1 wraps to list.length - 1).
        /// For example, 0 % 3 = 0, -1 % 3 = -1, -2 % 3 = -2, -3 % 3 = 0, and so forth, but WrapTo(0, 3) = 0,
        /// WrapTo(-1, 3) = 2, -2 % 3 = 1, -3 % 3 = 0, and so forth.  This can be useful if you're trying to
        /// "wrap" a number around at both ends, for example wrap to 3, such that 3 wraps to 0, and 0 wraps to 2.
        /// This is common if you are wrapping around an array index to the length of the array and need to ensure that positive numbers
        /// greater than or equal to the length of the array wrap to the beginning of the array (index 0), AND that negative numbers (under 0)
        /// wrap around to the end of the array (Length - 1).  In that example to wrap a number i you would call WrapTo(i, array.Length).
        /// </summary>
        /// <param name="num">The number to wrap.</param>
        /// <param name="wrapTo">The number to wrap to -- the result of the function is as outlined in function description, and guaranteed to
