namespace GoRogue
{
    ...
    /// </summary>
    /// <remarks>
    /// Many roguelike/2D grid algorithms, such as pathfinding, FOV, and map generation, view a map as simply
    /// a 2D array of some type.  In many games, however, the "value" associated with each 2D position may be
    /// dependent upon many different things.  For example, pathfinding, as input, often needs a "walkability map" --
    /// in common terms, a 2D array of bools where the boolean value at each position represents whether or not that
