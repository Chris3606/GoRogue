...
{
    /// <summary>
    /// See SpatialMap documentation -- similar in principle.  However, this implementation allows multiple
    /// items to exist at one point in the SpatialMap, in exchange for the loss of the convenience functions like
    /// GetItem vs GetItems, as well as potential performance differences (although unless the number of objects
    /// at any given location is large, the performance is asymptotically the same).
    /// </summary>
    /// <remarks>
    /// Although SpatialMap should generally be preferred in cases where only one item is allowed at a location in the
    ...
    public class MultiSpatialMap<T> : ISpatialMap<T> where T : IHasID
    {
        ...
        /// </summary>
        /// <param name="initialCapacity">The initial maximum number of elements the MultiSpatialMap can hold before it has to internally resize data structures.  Defaults to 32.</param>
        ...
        /// <summary>
        /// Adds the given item at the given position, provided the item is not already in the MultiSpatialMap.
        /// If the item is already contained in it, does nothing and returns false.  Otherwise (if item was successfully
        /// added), returns true.
        /// </summary>
        ...
        /// <summary>
        /// Move the item specified to the position specified.  Returns true if successful.  If the item does not exist in the MultiSpatialMap, does nothing
        ...
        /// <summary>
        /// Moves everything at position current, if anything, to postion target.  If something was moved, returns everything that was moved.  If nothing was moved, eg. there was nothing at position
        ...
        /// <returns>The items removed, if any were removed; nothing if no item was found at that position.</returns>
