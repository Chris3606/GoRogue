...
{
	/// <summary>
	/// Base class for a map that consists of one or more objects of type BaseObject. It implements
	/// basic functionality to manage these objects, as well as commonly needed functinonality like
	/// tile exploration, FOV, and pathfinding. BaseObject must be a type that derives from GameObject&lt;BaseObject&gt;.
	/// </summary>
	/// <remarks>
	/// A Map consists of BaseObjects on one or more layers. These layers are numbered, from the
	/// lowest layer of 0 upward. Each Map contains at minimum a "terrain" layer. This is considered
	/// to be layer 0. All objects added to this layer must have their IsStatic flag set to true, and
	/// must reside on layer 0. /// A map will typically also have some other layers, for non terrain
	/// objects like the player, monsters, items, etc. The number of these layers present on the map,
	/// along with which of all the layers participate in collision detection, etc., can be specified
	/// in the constructor. /// While this class has some flexibility, it does, unlike the rest of
	/// the library, tend to impose itself on your architecture. In cases where this is undesireable,
	/// each component of this map class exists as a separate component (layer masking, the
	/// SpatialMap(s) storing the entity layers, FOV, and pathfinding all exist as their own (more
	/// flexible) components). This class is not intended to cover every possible use case, but
	/// instead may act as an example or starting point in the case where you would like to use the
	/// components in a different way or within a different architecture.
	/// </remarks>
	/// <typeparam name="BaseObject">
	/// The class deriving from GameObject that this map will hold.
	/// </typeparam>
	public class Map<BaseObject> : IMapView<IEnumerable<BaseObject>> where BaseObject : GameObject<BaseObject>
	{
		/// <summary>
		/// Whether or not each tile is considered explored. Tiles start off unexplored, and become
		/// explored as soon as they are within a calculated FOV. This ArrayMap may also have values
		/// set to it, to easily allow for serialization or wizard-mode like functionality.
		/// </summary>
		...
		/// Layer mask containing those layers that should be allowed to have items that block
		/// walkability. Defaults to all layers.
		/// </param>
		...
		/// Layer mask containing those layers that should be allowed to have multiple objects at the
		/// same location on the same layer. Defaults to no layers.
		/// </param>
		...
		/// <summary>
		/// Constructor. Constructs map with the given terrain layer, determining width/height based
		/// on the width/height of that terrain layer.
		/// </summary>
		/// <param name="terrainLayer">
		/// The ISettableMapView that represents the terrain layer for this map. It is intended that
		/// this be modified ONLY via the SetTerrain function -- if it is modified outside of that,
		/// the ItemAdded/Removed events are NOT guaranteed to be called!
		/// </param>
		/// <param name="numberOfEntityLayers">Number of non-terrain layers for the map.</param>
		/// <param name="distanceMeasurement">
		/// Distance measurement to use for pathing/measuring distance on the map.
		...
		/// <summary>
		/// Layer mask that contains only layers that block transparency. A non-transparent
		/// BaseObject can only be added to this map if the layer it is on is contained within this
		/// layer mask.
		/// </summary>
		public uint LayersBlockingTransparency { get; }

		/// <summary>
		/// Layer mask that contains only layers that block walkability. A non-walkable BaseObject
		...
		/// <summary>
		/// IMapView representing transparency values for each tile. Each location is true if the
		/// location is transparent (there are no non-transparent objects at that location), false otherwise.
		/// </summary>
		public IMapView<bool> TransparencyView { get; }

		/// <summary>
		/// IMapView representing walkability values for each tile. Each location is true if the
		/// location is walkable (there are no non-walkable objects at that location), false otherwise.
		/// </summary>
		...
		/// <summary>
		/// Gets all objects at the given location, from the highest layer (layer with the highest
		/// number) down.
		/// </summary>
		...
		/// <summary>
		/// Sets the terrain at the given objects location to the given object, overwriting any
		/// terrain already present there.
		/// </summary>
		...
		/// <summary>
		/// Adds the given entity to its recorded location, removing it from the map it is currently
		/// a part of. Returns true if the entity was added, false otherwise (eg., collision
		/// detection would not allow it, etc.)
		/// </summary>
		...
		/// <summary>
		/// Removes the given entity from the map, returning true if it was successfully removed,
		/// false otherwise.
		/// </summary>
		/// <param name="entity">The entity to remove from the map.</param>
		/// <returns>
		/// True if the entity was removed successfully, false otherwise (eg, the entity was not part
		/// of this map).
		/// </returns>
		...
		/// <summary>
		/// Gets the first object encountered at the given position, moving from the highest existing
		/// layer in the layer mask downward. Layer mask defaults to all layers.
		/// </summary>
		...
		/// <summary>
		/// Gets all objects encountered at the given position, in order from the highest existing
		/// layer in the layer mask downward. Layer mask defaults to all layers.
		/// </summary>
		...
		/// </param>
		/// <returns>
		/// All objects encountered at the given position, in order from the highest existing layer
		...
		/// <summary>
		/// Calculates FOV with the given center point and radius (of shape circle), and stores the
		/// result in the FOV property. All tiles that are in the resulting FOV are marked as explored.
		/// </summary>
		...
		/// <summary>
		/// Calculates FOV with the given center point and radius, and stores the result in the FOV
		/// property. All tiles that are in the resulting FOV are marked as explored.
		/// </summary>
		/// <param name="position">The center point of the new FOV to calculate.</param>
		/// <param name="radius">The radius of the FOV. Defaults to infinite.</param>
		/// <param name="radiusShape">
		/// The shape of the FOV to calculate. Can be specified as either Distance or Radius types
		...
		/// <summary>
		/// Calculates FOV with the given center point and radius, restricted to the given angle and
		/// span, and stores the result in the FOV property. All tiles that are in the resulting FOV
		...
		/// <param name="span">
		/// The angle in degrees specifying the full arc of the FOV cone. span/2 degrees on either
		/// side of the given angle are included in the cone.
		/// </param>
