...
namespace GoRogue.MapGeneration.Steps
{
    ...
    /// to place connections on (within the parameters specified).  For each side, it then selects randomly from the valid connection points on that side, and
    /// carves out the selected location by setting its value to true in the "WallFloor" map view, and adding it to the list of doors associated with the
    /// appropriate room in the <see cref="DoorList"/> context component.  It continues to select connection points on a side until the <see cref="CancelConnectionPlacementChance"/>
    /// succeeds.  <see cref="CancelConnectionPlacementChance"/> is increased by <see cref="CancelConnectionPlacementChanceIncrease"/> each time a point is selected.
    ///
    ...
    public class RoomDoorConnection : GenerationStep
    {
        ...
        /// <summary>
        /// Increase the <see cref="CancelConnectionPlacementChance"/> value by this amount each time a door is placed (per side). Defaults to 10.
        /// </summary>
        ...
        /// <param name="roomsComponentTag">Optional tag that must be associated with the component that contains the rooms being connected by this algorithm.  Defaults to "Rooms".</param>
        ...
        /// <param name="doorsListComponentTag">Optional tag that must be associated with the component created/used to record the locations of doors created by this algorithm.  Defaults to "Doors".</param>
        public RoomDoorConnection(string? name = null, string? roomsComponentTag = "Rooms", string? wallFloorComponentTag = "WallFloor", string? doorsListComponentTag = "Doors")
            : base(name, (typeof(ItemList<Rectangle>), roomsComponentTag), (typeof(ISettableMapView<bool>), wallFloorComponentTag))
        {
            ...
            foreach (var room in rooms.Items)
            {
                ...
                foreach (var side in AdjacencyRule.Cardinals.DirectionsOfNeighbors())
                {
                    foreach (var sidePosition in room.PositionsOnSide(side))
                    {
                        ...
                        var testPoint = wallPoint + side; // Keep going in that direction to see where an opening here would lead
                ...
                var validSides = AdjacencyRule.Cardinals.DirectionsOfNeighbors().Where(side => validPositions[side].Count > 0).ToList();
                ...
                {
                    int sidesRemoved = 0;
                    for (int i = 0; i < validSides.Count; i++)
                    {
                ...
                foreach (var side in validSides)
                {
                    ...
                    while (validPositions[side].Count > 0)
                    {
                        ...
                        // previously
                        if (AdjacencyRule.Cardinals.Neighbors(newConnectionPoint).Count(pos => wallFloor.Contains(pos) && !wallFloor[pos]) >= 2)
                        {
