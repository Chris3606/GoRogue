...
namespace GoRogue.MapGeneration.Steps.Translation
{
    ...
    public class AppendItemLists<TItem> : GenerationStep
    {
        ...
        /// <param name="baseListTag">A tag that must be attached to the component that will have items from the other list appended onto it.</param>
        /// <param name="listToAppendTag">A tag that must be attached to the component that will have its items appended onto the base list.</param>
            ...
            : base(name, (typeof(ItemList<TItem>), baseListTag), (typeof(ItemList<TItem>), listToAppendTag))
        {
            ...
            if (BaseListTag == ListToAppendTag)
                throw new InvalidConfigurationException(this, nameof(BaseListTag), $"An ItemList cannot be appended to itself, so the base tag must be different than the {nameof(ListToAppendTag)}.");
        }
        ...
        protected override void OnPerform(GenerationContext context)
        {
            ...
            var baseList = context.GetComponent<ItemList<TItem>>(BaseListTag)!; // Not null because is in required components list
            var listToAppend = context.GetComponent<ItemList<TItem>>(ListToAppendTag)!; // Not null because is in required components list
